        ...
        public void ResizeImageFromFile(string OriginalFileLocation, int heigth, int width, Boolean keepAspectRatio, Boolean getCenter)
        {
            ...
            if (keepAspectRatio || getCenter)
            {
                ...
                if (getCenter)
                {
                    ...
                    // create the section to cut of the original image
                    // System.Console.WriteLine("the section that will be cut off: " + section.Size.ToString() + " the Y value is minimized by: " + bmpY);
                    Bitmap orImg = new Bitmap((Bitmap)FullsizeImage);
                    // for the correct effect convert image to bitmap.
                    ...
                    using (Bitmap tempImg = new Bitmap(section.Width, section.Height))
                    {
                        ...
                        orImg.Dispose();
                        cutImg.Dispose();
                        FullsizeImage.GetThumbnailImage(width, heigth, null, IntPtr.Zero).Save(OriginalFileLocation, ImageFormat.Png);
                    }
                }
                else newheigth = (int)(FullsizeImage.Height / resize);// set the new heigth of the current image
            }// return the image resized to the given heigth and width
            FullsizeImage.GetThumbnailImage(width, newheigth, null, IntPtr.Zero).Save(OriginalFileLocation, ImageFormat.Png);
        }
    }
